...
func thread_execution(data_slice *[]*Profile, waitgroup * sync.WaitGroup, profile_compare *Profile, start_idx int, end_idx int, dist_fn func(*[]int, *[]int) float64){
	...
	*/
	...
	for i := start_idx; i < end_idx; i++ {
		...
		fmt.Fprintf(os.Stdout, "ID1: %s ID2: %s value: %.3f\n", profile_compare.name, (*data_slice)[i].name, x);
	}
...
func run_data(profile_data *[]*Profile) {
	...
	*/
	...
	dist := select_dist_func() // ! Default value is stored in the dists.go file
	...
	for g := range data[0:] {
		...
		for i := bucket_index; i < len(buckets); i++ {
			wg.Add(1)
			//bucket_range := buckets[bucket_index][1] - bucket_size
			go thread_execution(&data, &wg, profile_comp, buckets[i][0], buckets[i][1], dist)
		}
